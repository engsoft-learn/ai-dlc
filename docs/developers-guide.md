...
**Download:** [developers-guide.pdf](pdf/developers-guide.pdf)
...
The `security-scanners.yml` workflow runs six scanners on every push to `main`, every PR targeting `main`, and on a daily schedule. Each scanner uploads a SARIF report to GitHub Code Scanning (visible under the **Security** tab) and as a downloadable artifact.
...
**What triggers failure:** Any finding with **high confidence**, at any severity level. See the Bandit configuration in `.github/workflows/security-scanners.yml` for the exact filters used.

**Scope:** Runs against all tracked Python files in the repository; see `.github/workflows/security-scanners.yml` for the precise include/exclude patterns.
